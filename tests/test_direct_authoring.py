...
def test_canonical_course_manifest_sources_and_links_are_current():
    ...
    assert manifest["course_id"] == 4066
    assert len(manifest["objects"]) == 46
    assert len({item["source"] for item in manifest["objects"]}) == 45
...
def test_authoritative_sources_propagate_into_imscc(tmp_path):
    ...
    with zipfile.ZipFile(result["output"]) as archive:
        assert archive.read("course_settings/syllabus.html").decode() == (ROOT / "course/content/syllabus.html").read_text()
